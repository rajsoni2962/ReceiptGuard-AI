...
export const ReceiptDetailModal: React.FC<ReceiptDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-6xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/70">
          ...
          <div className="flex items-center space-x-2">
            ...
            <a
              ...
              className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-emerald-400 hover:text-emerald-300 bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-700/50 transition"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-4">
              ...
              <p className="text-sm text-slate-400">Loading original document & protection details...</p>
            </div>
          ) : error ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-6 flex flex-col space-y-3">
                ...
                <div className="relative rounded-xl border border-slate-800 bg-slate-950/90 overflow-hidden flex items-center justify-center min-h-[480px] max-h-[620px]">
                  {isPdf ? (
                    <iframe
                      src={fileViewUrl}
                      title="Original PDF Preview"
                      className="w-full h-[580px] rounded-lg border-0 bg-slate-900"
                    />
                  ) : isImage ? (
                    <div className="p-4 flex items-center justify-center w-full h-full overflow-auto">
                      <img
                        ...
                        className="max-h-[540px] max-w-full object-contain rounded-lg shadow-lg border border-slate-800"
                      />
                    </div>
                  ) : (
                    <div className="p-8 text-center space-y-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-white">{receipt.original_filename || receipt.filename}</p>
                        <p className="text-xs text-slate-400 mt-1">Text-based purchase document</p>
                      </div>
                      {receipt.raw_text && (
                        <div className="text-left max-h-64 overflow-y-auto p-4 rounded-lg bg-slate-900 border border-slate-800 font-mono text-xs text-slate-300 whitespace-pre-wrap">
                          {receipt.raw_text}
                        </div>
                      )}
                      <a
              ...
              <div className="lg:col-span-6 flex flex-col space-y-5">
                ...
                <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-bold text-white">{receipt.store}</h3>
                      {receipt.store_address && (
                        <p className="text-xs text-slate-400">{receipt.store_address}</p>
                      )}
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-slate-400 font-mono">Grand Total</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 pt-2 border-t border-slate-800/80 text-xs">
                    <div>
                      ...
                      <span className="text-slate-200 font-medium">{receipt.purchase_date}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[10px] uppercase font-semibold">Invoice No</span>
                      <span className="text-slate-200 font-mono font-medium">{receipt.invoice_number || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[10px] uppercase font-semibold">
                        {receipt.sale_id ? 'Sale ID' : 'Order ID'}
                      </span>
                      <span className="text-slate-200 font-mono font-medium">{receipt.sale_id || receipt.order_id || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[10px] uppercase font-semibold">Customer</span>
                      <span className="text-slate-200 font-medium">{receipt.customer_name || 'Raj Soni'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[10px] uppercase font-semibold">Payment</span>
                      <span className="text-slate-200 font-medium">{receipt.payment_method || 'UPI / Card'}</span>
                    </div>
                    <div>
                    ...
                    <div className="pt-3 border-t border-slate-800/80 space-y-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                        {receipt.invoice_sections.map((sec, idx) => (
                          <div key={idx} className="p-2 rounded-lg bg-slate-900 border border-slate-800 flex flex-col space-y-0.5">
                            <div className="flex justify-between font-semibold text-slate-200 capitalize">
                              <span>Page {sec.page_number}: {sec.section_type.replace('_', ' ')}</span>
                              <span className="text-emerald-400 font-mono">{formatCurrency(sec.grand_total, receipt.currency)}</span>
                            </div>
                            <span className="text-[10px] text-slate-400 truncate">{sec.seller_name}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {receipt.calculations?.map((calc) => (
                      ...
                      >
                        <div className="flex items-center justify-between font-semibold">
                          <span className="text-white">{calc.item_name}</span>
                          <span className="font-mono text-slate-300">{formatCurrency(calc.price, receipt.currency)}</span>
                        </div>
                        <div className="flex items-center justify-between mt-1 text-[11px]">
                          ...
                          {calc.applicable_policy_section && (
                            <span className="text-slate-400 font-mono text-[10px]">{calc.applicable_policy_section}</span>
                          )}
                        </div>
                        {calc.warranty_days_remaining !== null && calc.warranty_days_remaining !== undefined && (
                          <div className="mt-1 text-[11px] text-indigo-300 flex items-center space-x-1">
                ...
                <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-2.5">
                  ...
                  <div className="pt-2 border-t border-slate-800 text-xs space-y-1 text-slate-400">
                    <div className="flex justify-between">
                      <span>Subtotal</span>
                      <span className="font-mono text-slate-300">{formatCurrency(receipt.subtotal, receipt.currency)}</span>
                    </div>
                    ...
                    <div className="flex justify-between">
                      <span>Tax / GST</span>
                      <span className="font-mono text-slate-300">{formatCurrency(receipt.tax_total, receipt.currency)}</span>
                    </div>
                    <div className="flex justify-between font-bold text-white pt-1 border-t border-slate-800/60">
                      <span>Grand Total</span>
                      <span className="font-mono text-emerald-400">{formatCurrency(receipt.grand_total, receipt.currency)}</span>
                    </div>
                  </div>
                </div>

                {/* Source & Vault Verification */}
                <div className="p-3.5 rounded-xl bg-slate-950/40 border border-slate-800/80 text-[11px] text-slate-400 flex items-center justify-between">
                  <div className="space-y-0.5">
                    <p><strong className="text-slate-300">Vault Document:</strong> {receipt.original_filename || receipt.filename}</p>
                    <p><strong className="text-slate-300">Method:</strong> {receipt.extraction_method} • OCR Confidence: {Math.round((receipt.ocr_confidence || 1) * 100)}%</p>
                  </div>
                  {onAskReceiptGuard && (
                    <button
                      onClick={() => {
                      ...
                      className="inline-flex items-center space-x-1.5 px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold shadow-md shadow-indigo-600/30 transition text-xs"
                    >
